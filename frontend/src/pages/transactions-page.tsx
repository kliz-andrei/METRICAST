...
export function TransactionsPage() {
  ...
  if (q.isError)
    return (
      <ErrorState message="Unable to load transactions. Please try again." />
    );
  const data = q.data;
  ...
  return (
    <section>
      ...
      <p className="mt-2 text-slate-500 dark:text-slate-400">
        Search and review imported POS transactions.
      </p>
      <div className="my-6 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 md:grid-cols-3">
        ...
        >
          ...
          <option value="guestCount">Guest Count</option>
        </select>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ["Transactions", data.summary.totalTransactions.toLocaleString()],
          ["Net Sales", money(data.summary.netSales)],
          ["Average Order Value", money(data.summary.averageOrderValue)],
          ["Guests", data.summary.guests.toLocaleString()],
        ].map(([a, b]) => (
          <article key={a} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <tbody>
            {data.transactions.map((t) => (
              <tr
                ...
                className="cursor-pointer border-t border-slate-100 text-slate-800 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-100 dark:hover:bg-slate-800/70"
              >
                <td className="p-3">
                ...
                <td className="p-3">
                  {new Date(t.occurredAt).toLocaleString("en-PH")}
                </td>
                <td className="p-3">{t.orderType}</td>
                <td className="p-3">{t.salesChannel}</td>
                <td className="p-3">{t.guestCount ?? 0}</td>
                <td className="p-3">{money(t.grossSales)}</td>
                <td className="p-3">{money(t.discountAmount)}</td>
                <td className="p-3">{money(t.serviceCharge)}</td>
                <td className="p-3">{money(t.netSales)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {selected && (
        ...
        >
          <article
            className="mx-auto max-w-2xl rounded-2xl bg-white p-6 text-slate-900 dark:bg-slate-900 dark:text-slate-100"
            onClick={(e) => e.stopPropagation()}
          >
            {d.isLoading ? (
              <LoadingSkeleton className="h-64" />
            ) : d.isError ? (
              <ErrorState message="Unable to load transaction details." />
            ) : (
              d.data && (
                <>
                  <button
                  ...
                  <h3 className="text-xl font-bold">
                    Transaction {d.data.sourceTransactionId}
                  </h3>
                  <p>
                    {d.data.invoiceNo} · {d.data.orderType} ·{" "}
                    {d.data.salesChannel}
                  </p>
                  <h4 className="mt-5 font-semibold">Payments</h4>
                  {d.data.payments.length ? (
                    d.data.payments.map((p) => (
                      <p key={p.id}>
                        {p.paymentMethod}: {money(Number(p.amount))}
                      </p>
                    ))
                  ) : (
                    <p>No payment record available</p>
                  )}
                  <h4 className="mt-5 font-semibold">Product Sales</h4>
                  {d.data.items.length ? (
                    d.data.items.map((i) => (
                      <p key={i.id}>
                        {i.product.name} · {i.quantity} ×{" "}
                        {money(Number(i.unitPrice))} ·{" "}
                        {money(Number(i.salesAmount))}
                      </p>
                    ))
                  ) : (
                    <p>No product sales record available</p>
                  )}
                </>
              )
            )}
